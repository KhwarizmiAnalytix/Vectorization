...
def _raise_stack_limit() -> None:
    ...
    (-O1 and above) on large translation units (expression-template headers,
    vendored googletest), crashing with an internal "stack smashing detected" /
    instruction-selection segfault. Raising RLIMIT_STACK before spawning the
    build (inherited by ninja's clang child processes) avoids the crash.
    """
    if resource is None:
        return
    try:
